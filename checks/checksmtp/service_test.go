...
func TestService_Create(t *testing.T) {
	...
	svc := New(mock)
	resp, err := svc.Create(context.Background(), &Request{Name: "smtp-check"})

	require.NoError(t, err)
	assert.Equal(t, 1, resp.ID)
	require.Len(t, mock.Calls, 1)
	assert.Equal(t, http.MethodPost, mock.Calls[0].Method)
	assert.Equal(t, "/rmon/check/smtp", mock.Calls[0].Path)
}

func TestService_Get(t *testing.T) {
	mock := &testutil.MockRequester{}
	mock.RespondWith(Response{Name: "smtp-check"})

	svc := New(mock)
	resp, err := svc.Get(context.Background(), 5)

	require.NoError(t, err)
	assert.Equal(t, "smtp-check", resp.Name)
	require.Len(t, mock.Calls, 1)
	assert.Equal(t, http.MethodGet, mock.Calls[0].Method)
	assert.Equal(t, "/rmon/check/smtp/5", mock.Calls[0].Path)
}
